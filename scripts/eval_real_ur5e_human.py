...
from ti.real_world.aruco import get_marker_pose
...
@click.command()
@click.option('--input', '-i', default='/home/haonan/Projects/direct_human_demo/data/outputs/2025.02.10/20.55.52_train_diffusion_policy_real_hammer_human/checkpoints/latest.ckpt', help='Path to checkpoint')
@click.option('--output', '-o', default='/home/haonan/Projects/direct_human_demo/data/random_output', help='Directory to save recording')
@click.option('--robot_left_ip', '-ri', default='192.168.0.3', help="UR5's IP address e.g. 192.168.0.204")
@click.option('--robot_right_ip', '-ri', default='192.168.0.2', help="UqR5's IP address e.g. 192.168.0.204")
...
@click.option('--use_gripper', '-ug', default=False, type=bool)
@click.option('--ctrl_mode', '-cm', default='eef', type=str)
@click.option('--relative_transform', '-r', default=True, type=bool)
@click.option('--debug', '-d', default=False, type=bool)
def main(input, output, robot_left_ip, robot_right_ip, match_dataset, match_episode,
    vis_camera_idx, init_joints, 
    steps_per_inference, max_duration,
    frequency, command_latency,
    bimanual,use_gripper, ctrl_mode, 
    relative_transform, debug):
    # load match_dataset
    match_camera_idx = 0
    ...
    mesh = trimesh.load(cfg.task.mesh_file)
    ...
    if 'camera_features' in cfg.task.shape_meta.obs:                        
        real_obs_info['mast3r_model'] = None 
    ...
    gt_cam_key = f'camera_{vis_camera_idx}' 
    base_pose_cam_key = f'camera_0'
    use_task_frame = False
    ...
    with SharedMemoryManager() as shm_manager:
        with Spacemouse(shm_manager=shm_manager,
            ...
            tool_in_eef_path = "tool_in_eef.npy"  
            ...
            with torch.no_grad():
                ...
                if use_task_frame:
                    init_base_pose, base_pose_img_bgr = get_marker_pose(obs[base_pose_cam_key+'_color'][-1][...,::-1].copy(), obs[base_pose_cam_key+'_intrinsics'][-1])
                if relative_transform:
                    tool_pose, pose_rgb = get_tool_in_cam(pose_model, cfg.task.segmentation_labels, obs=obs, shape_meta=cfg.task.shape_meta,to_origin=to_origin, bbox=bbox, camera_key=gt_cam_key, init=True, grounding_dino_model=grounding_dino_model, sam_predictor=sam_predictor)
                    init_tool_pose = np.repeat(tool_pose[np.newaxis, :, :], obs_dict_np['robot_eef_pose'].shape[0], axis=0)
                    init_tool_pose = _homo_to_9d_action(init_tool_pose, rotation_transformer)
                    
                    cam_in_base = np.linalg.inv(obs['robot_base_pose_in_world'][-1])@obs[gt_cam_key+'_extrinsics'][-1]
                    tool_in_base = cam_in_base @ tool_pose 
                    init_tool_in_base = np.repeat(tool_in_base[np.newaxis, :, :], obs_dict_np['robot_eef_pose'].shape[0], axis=0)
                    obs_dict_np['robot_eef_pose'] = _homo_to_9d_action(init_tool_in_base, rotation_transformer)
                ...
                result = policy.predict_action(obs_dict, vis_tracks=True)

                action_9d = result['action'][0].detach().to('cpu').numpy()
                try:
                    tool_in_eef = np.load(tool_in_eef_path)
                    print(f"tool_in_eef transformation loaded successfully:\n{tool_in_eef}")
                except FileNotFoundError:
                    print(f"File not found: {tool_in_eef_path}")
                    print("Calculating tool_in_eef transformation")
                    tool_in_eef = np.linalg.inv(_6d_axis_angle_to_homo(obs['robot_eef_pose'])[-1])@np.linalg.inv(obs['robot_base_pose_in_world'][-1])@obs[f'{gt_cam_key}_extrinsics'][-1]@tool_pose


                action = policy_action_to_env_action(action_9d, obs['robot_eef_pose'][-1], init_tool_pose[-1], tool_in_eef, action_mode=ctrl_mode)
            ...
            while True:
                ...
                while True:
                    ...
                    obs = env.get_obs()
                    if use_task_frame:
                        base_pose, base_pose_img_bgr = get_marker_pose(obs[base_pose_cam_key+'_color'][-1][...,::-1].copy(), obs[base_pose_cam_key+'_intrinsics'][-1])
                        rel_base_trans = np.linalg.inv(init_base_pose['homogeneous_matrix'])@base_pose['homogeneous_matrix']
                    else:
                        rel_base_trans = np.eye(4)
                    ...
                    if key_stroke == ord('q'):
                        ...
                        break
                    elif key_stroke == ord('t'):
                        obs = env.get_obs()
                        tool_pose, pose_rgb = get_tool_in_cam(pose_model, cfg.task.segmentation_labels, obs=obs, shape_meta=cfg.task.shape_meta,to_origin=to_origin, bbox=bbox, camera_key=gt_cam_key, init=True, grounding_dino_model=grounding_dino_model, sam_predictor=sam_predictor)
                        tool_in_eef = np.linalg.inv(_6d_axis_angle_to_homo(obs['robot_eef_pose'])[-1])@np.linalg.inv(obs['robot_base_pose_in_world'][-1])@obs[f'{gt_cam_key}_extrinsics'][-1]@tool_pose
                        np.save(tool_in_eef_path, tool_in_eef)
                        print(f"tool_in_eef transformation: {tool_in_eef} saved to {tool_in_eef_path}")

                        cv2.imshow("Pose RGB Visualization", pose_rgb[...,::-1])
                    ...
                    if bimanual == False:
                        ...
                        if intermediate_pose != []:
                            ...
                            target_pose_homo = _6d_axis_angle_to_homo(target_pose)
                            target_pose_homo = np.linalg.inv(rel_base_trans)@target_pose_homo
                            target_pose = _homo_to_6d_axis_angle(target_pose_homo)
                ...
                try:
                    ...
                    while True:
                        ...
                        with torch.no_grad():
                            ...
                            if relative_transform:
                                if debug:
                                    tool_pose, pose_rgb = get_tool_in_cam(pose_model, cfg.task.segmentation_labels, obs=obs, shape_meta=cfg.task.shape_meta,to_origin=to_origin, bbox=bbox, camera_key=gt_cam_key, init=True if iter_idx==0 else False, grounding_dino_model=grounding_dino_model, sam_predictor=sam_predictor)
                                    tool_in_eef = np.linalg.inv(_6d_axis_angle_to_homo(obs['robot_eef_pose'])[-1])@np.linalg.inv(obs['robot_base_pose_in_world'][-1])@obs[f'{gt_cam_key}_extrinsics'][-1]@tool_pose
                                else:
                                    pose_rgb = obs[f'{gt_cam_key}_color'][-1]

                                init_tool_in_base = _9d_to_homo_action(obs_dict_np['robot_eef_pose'])@tool_in_eef
                                init_tool_in_base = _homo_to_9d_action(init_tool_in_base, rotation_transformer)


                                obs_dict_np['robot_eef_pose'] = init_tool_in_base
                                init_tool_pose = [None] 
                            ...
                            action_9d = result['action'][0].detach().to('cpu').numpy()
                            if debug:
                                homo_actions = _9d_to_homo_action(action_9d)
                                homo_actions = np.linalg.inv(cam_in_base)@homo_actions

                            action = policy_action_to_env_action(action_9d, obs['robot_eef_pose'][-1], init_tool_pose[-1], tool_in_eef, action_mode=ctrl_mode)
                            action_homo = _6d_axis_angle_to_homo(action)
                            correct_action_homo = np.linalg.inv(rel_base_trans)@action_homo
                            action = _homo_to_6d_axis_angle(correct_action_homo)
                        ...
                        episode_id = env.replay_buffer.n_episodes
                        if relative_transform:
                            vis_img = pose_rgb if pose_rgb is not None else obs[f'camera_{vis_camera_idx}_color'][-1]
                        else:
                            vis_img = obs[f'camera_{vis_camera_idx}_color'][-1]
