...
import yaml
from ti.real_world.bimanual_rtde import BimanualRTDEInterpolationController
...
class RealUR5eEnv:
    def __init__(self,
        ...
        RobotController = RTDEInterpolationController if not bimanual else BimanualRTDEInterpolationController
        if not bimanual:
            robot_ip = robot_left_ip if single_arm_type == 'left' else robot_right_ip
        else:
            robot_ip = (robot_left_ip, robot_right_ip)
...
def test_env_demo_replay(ctrl_mode = 'joint'):
    d = dirname(dirname(dirname(abspath(__file__))))
    zarr_path = os.path.join(d, 'data/hammer_robot/replay_buffer.zarr')
    ...
    not_moving_mask = are_joints_close(demo_dict['observations']['robot_joint'], demo_dict['joint_action'])
    moving_mask = ~not_moving_mask
    episode_slice = slice(episode_start, episode_end)
    episode_mask = moving_mask[episode_slice]

    if ctrl_mode == 'joint':
        # actions = demo_dict['joint_action'][episode_slice][episode_mask]
        actions = demo_dict['observations/robot_joint'][episode_slice][episode_mask]

    else:
        # actions = demo_dict['cartesian_action'][episode_start:episode_end]
        actions = demo_dict['observations/robot_eef_pose'][episode_slice][episode_mask]
        ...
        rotation_6d = rotation_transformer1.forward(actions[:,3:])        
        actions[:,3:] = rotation_transformer2.forward(rotation_6d)
        ...
        while True:
            ...
            if ctrl_mode == 'joint':
                env.exec_actions(
                    ...
                    eef_actions=np.zeros((action_batch.shape[0], 6)),
...
def test_env_human_replay():
    ...
    with RealUR5eEnv(
        ...
        timestamps = time.time() + np.arange(len(tool_actions)) / frequency + 1.0
        start_step = 0
        ...
        while True:
            curr_time = time.monotonic()
            loop_end_time = curr_time + 1.0
            end_step = min(start_step+frequency, len(robot_action_6d))
            ...
            for eef_action in action_batch:
                env.robot.servo_ee_pose(eef_action, duration=duration)
                time.sleep(duration)
