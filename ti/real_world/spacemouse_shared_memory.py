...
class Spacemouse(mp.Process):
    def __init__(self, 
            ...
            n_buttons=2,
            verbose=False,
            bimanual=False,# dummy
            use_gripper=False,# dummy
            ):
        """
    ...
    def run(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                event = spnav_poll_event() # If no event is available, the function returns None, otherwise it returns an event.
                receive_timestamp = time.time()
...
def test_space_mouse_input():
    with SharedMemoryManager() as shm_manager:
        with Spacemouse(shm_manager=shm_manager, 
                        bimanual=False,
                        use_gripper=False)  as device:
        
            while 1:
                print(device.get_motion_state_transformed())
        
if __name__ == '__main__':
    test_space_mouse_input()