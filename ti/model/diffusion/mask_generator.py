...
from ti.model.common.module_attr_mixin import ModuleAttrMixin
...
class LowdimMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        device = self.device
        B, T, D = shape
        assert D == (self.output_dim + self.obs_dim)
        ...
        is_output_dim = dim_mask.clone()
        is_output_dim[...,:self.output_dim] = True
        is_obs_dim = ~is_output_dim
        ...
        if self.action_visible:
            ...
            action_mask = action_mask & is_output_dim
...
class KeypointMaskGenerator(ModuleAttrMixin):
    ...
    def forward(self, shape, seed=None):
        device = self.device
        B, T, D = shape
        all_keypoint_dims = D - self.output_dim - self.context_dim
        ...
        is_obs_dim = ~(is_output_dim | is_context_dim)
        ...
        if self.time_independent:
            ...
            visible_dims_mask = torch.cat([
                torch.ones((B, T, self.output_dim), 
                    dtype=torch.bool, device=device),
                visible_dims,
