...
from ti.model.common.dict_of_tensor_mixin import DictOfTensorMixin
...
class SingleFieldLinearNormalizer(DictOfTensorMixin):
    ...
    def unnormalize(self, x: Union[torch.Tensor, np.ndarray], action_slice=None) -> torch.Tensor:
        return _normalize(x, self.params_dict, forward=False, action_slice=action_slice)
...
def _normalize(x, params, forward=True, action_slice=None):
    ...
    if action_slice is not None:
        scale = scale[action_slice]
        offset = offset[action_slice]
