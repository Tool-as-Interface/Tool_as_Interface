...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    ...
    # # left arm
    # "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT8ISUTV-if00-port0": DynamixelRobotConfig(
    ...
    #     gripper_config=(6, 16.85078125, -24.94921875),
    # ),


    # # right arm
    # "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FT8ISVPD-if00-port0": DynamixelRobotConfig(
    #     joint_ids=(0, 1, 2, 3, 4, 5),
    #     joint_offsets=(
    #         2*np.pi/2, 
    #         5*np.pi/2, 
    ...
    #     ),
    #     joint_signs=(1, 1, -1, 1, 1, 1),
    #     gripper_config=(6, 287.90546875, 246.10546875),
    # ),
}
...
class GelloAgent(Agent):
    def __init__(
        ...
        if dynamixel_config is not None:
            ...
            gello_config_path = os.path.join(Path(__file__).parent.parent.parent, "gello.yaml")
            ...
            with open(gello_config_path) as stream:
                try:
                    ...
                    print(exc)
            # Assuming 'port' is defined and contains the correct port string
            port_config = gello_config['gello_config'].get(port)
            if port_config:
                # Instantiate DynamixelRobotConfig with the configuration for the given port
                config = DynamixelRobotConfig(
                    joint_ids=port_config['joint_ids'],
                    joint_offsets=[offset * np.pi / 180 for offset in port_config['joint_offsets']],  # Convert degrees to radians if necessary
                    joint_signs=port_config['joint_signs'],
                    gripper_config=tuple(port_config['gripper_config'])
                )
            else:
                print(f"No configuration found for port {port}")
